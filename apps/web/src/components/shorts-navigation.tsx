import { ChevronDown, ChevronUp } from "lucide-react";

type Props = {
  onPrev: () => void;
  onNext: () => void;
  hasPrev: boolean;
  hasNext: boolean;
};

export function ShortsNavigation({ onPrev, onNext, hasPrev, hasNext }: Props) {
  return (
    <div className="flex flex-col gap-2">
      <NavButton icon={ChevronUp} onClick={onPrev} disabled={!hasPrev} label="Previous" />
      <NavButton icon={ChevronDown} onClick={onNext} disabled={!hasNext} label="Next" />
    </div>
  );
}

type NavButtonProps = {
  icon: React.ComponentType<{ className?: string }>;
  onClick: () => void;
  disabled: boolean;
  label: string;
};

function NavButton({ icon: Icon, onClick, disabled, label }: NavButtonProps) {
  return (
    <button
      type="button"
      onClick={onClick}
      disabled={disabled}
      className="flex h-12 w-12 items-center justify-center rounded-full border border-border-strong bg-surface text-fg transition-colors hover:bg-surface-strong disabled:cursor-not-allowed disabled:opacity-40"
      aria-label={label}
    >
      <Icon className="h-6 w-6" />
    </button>
  );
}
